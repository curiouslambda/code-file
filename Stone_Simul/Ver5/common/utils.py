...
def plot_value_curve(deltas: List[float], save_path: str):
    """벨만 잔차 수렴 곡선 플롯 (Value Iteration Convergence)"""
    plt.figure(figsize=(12, 8))
    iterations = range(1, len(deltas) + 1)
    plt.plot(iterations, deltas, 'b-', linewidth=2, label='Bellman Residual')
    plt.xlabel('Iteration', fontsize=12)
    plt.ylabel('Max |V^{k+1} - V^k|', fontsize=12)
    plt.title('벨만 잔차 수렴 곡선 (Bellman Residual Convergence)', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend(fontsize=12)
    plt.yscale('log')  # 로그 스케일로 수렴 패턴 더 잘 보이게
    plt.tight_layout()
...
def plot_policy_margin_heatmap(V: np.ndarray, env, c: int, p_idx: int, a_try: int, b_try: int, c_try: int, save_path: str):
    """정책 마진 히트맵: Q_best - Q_2nd를 (A,B) 평면에 표시"""
    # p 값 계산
    p = env.min_prob + p_idx * env.prob_change

    # A, B 범위 설정 (물리적 제약 고려)
    max_attempts_per_category = 10  # env.max_slots와 동일
    max_a = min(max_attempts_per_category, max_attempts_per_category - a_try)  # a <= max_attempts_per_category - a_try
    max_b = min(max_attempts_per_category, max_attempts_per_category - b_try)  # b <= max_attempts_per_category - b_try

    margin_matrix = np.zeros((max_a + 1, max_b + 1))

    # 각 (a,b) 상태에 대해 Q 값 계산
    for a in range(max_a + 1):
        for b in range(max_b + 1):
            # 현재 상태
            current_state = State(a=a, b=b, c=c, p=p,
                                a_try=a_try, b_try=b_try, c_try=c_try)

            # 가능한 액션들에 대한 Q 값 계산
            q_values = []

            for action_idx, action in enumerate(['A', 'B', 'C']):
                if action == 'A' and a_try == 0:
                    continue
                if action == 'B' and b_try == 0:
                    continue
                if action == 'C' and c_try == 0:
                    continue
                ...
                # 다음 상태의 가치 함수 값
                success_idx = (success_state.a, success_state.b, success_state.c,
                             round((success_state.p - env.min_prob) / env.prob_change),
                             success_state.a_try, success_state.b_try, success_state.c_try)
                fail_idx = (fail_state.a, fail_state.b, fail_state.c,
                           round((fail_state.p - env.min_prob) / env.prob_change),
                           fail_state.a_try, fail_state.b_try, fail_state.c_try)

                # 인덱스 범위 체크
                success_idx = tuple(min(max(0, idx), V.shape[i] - 1) for i, idx in enumerate(success_idx))
                fail_idx = tuple(min(max(0, idx), V.shape[i] - 1) for i, idx in enumerate(fail_idx))

                success_value = V[success_idx] if not env.is_terminal(success_state) else env.get_reward(success_state)
                fail_value = V[fail_idx] if not env.is_terminal(fail_state) else env.get_reward(fail_state)

                # Q 값 계산 (γ=1 가정, p를 명시적 계산)
                current_p = env.min_prob + p_idx * env.prob_change
                q_value = current_p * success_value + (1 - current_p) * fail_value
                q_values.append(q_value)

            if len(q_values) >= 2:
                # Q_best와 Q_2nd 계산
                sorted_q = sorted(q_values, reverse=True)
                margin = sorted_q[0] - sorted_q[1] if len(sorted_q) > 1 else 0
            else:
                margin = 0

            margin_matrix[a, b] = margin

    # 히트맵 플롯
    plt.figure(figsize=(12, 10))
    sns.heatmap(margin_matrix, cmap='RdYlBu_r', center=0,
                xticklabels=range(max_b + 1), yticklabels=range(max_a + 1),
                annot=True, fmt='.3f', cbar_kws={'label': 'Q_best - Q_2nd'})

    plt.xlabel('B Success Count', fontsize=12)
    plt.ylabel('A Success Count', fontsize=12)
    title = f'정책 마진 히트맵 (Policy Margin Heatmap)\n'
    title += f'c={c}, p={p:.2f}, a_try={a_try}, b_try={b_try}, c_try={c_try}'
...
def plot_policy_decision_map(V: np.ndarray, env, c: int, p_idx: int, a_try: int, b_try: int, c_try: int, save_path: str):
    """정책 결정 맵: (A,B) 격자에 최적 액션을 색상으로 표시"""
    # p 값 계산
    p = env.min_prob + p_idx * env.prob_change
    ...
    policy_matrix = np.full((max_a + 1, max_b + 1), -1, dtype=int)

    # 각 (a,b) 상태에 대해 최적 액션 계산
    for a in range(max_a + 1):
        for b in range(max_b + 1):
            # 현재 상태
            current_state = State(a=a, b=b, c=c, p=p,
            ...
            policy_matrix[a, b] = best_action

    # 정책 맵 플롯
    plt.figure(figsize=(12, 10))

    # 색상 맵 정의 (A=블루, B=그린, C=오렌지)
    colors = ['#1f77b4', '#2ca02c', '#ff7f0e']  # 파란색, 초록색, 주황색
    cmap = plt.matplotlib.colors.ListedColormap(colors)

    # 마스크 생성 (불가능한 상태는 회색으로)
    mask = (policy_matrix == -1)
    policy_matrix_masked = np.ma.masked_where(mask, policy_matrix)

    sns.heatmap(policy_matrix_masked, cmap=cmap,
                xticklabels=range(max_b + 1), yticklabels=range(max_a + 1),
                cbar=False, linewidths=0.5, linecolor='black')

    # 범례 추가
    legend_elements = [
        plt.Rectangle((0,0),1,1, facecolor='#1f77b4', edgecolor='black', label='Action A'),
        plt.Rectangle((0,0),1,1, facecolor='#2ca02c', edgecolor='black', label='Action B'),
        plt.Rectangle((0,0),1,1, facecolor='#ff7f0e', edgecolor='black', label='Action C')
    ]
    plt.legend(handles=legend_elements, loc='upper right', fontsize=12)

    plt.xlabel('B Success Count', fontsize=12)
    plt.ylabel('A Success Count', fontsize=12)
    title = f'정책 결정 맵 (Policy Decision Map)\n'
    title += f'c={c}, p={p:.2f}, a_try={a_try}, b_try={b_try}, c_try={c_try}'
    plt.title(title, fontsize=14, fontweight='bold')
