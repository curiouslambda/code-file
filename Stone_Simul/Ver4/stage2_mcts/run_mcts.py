...
def run_episode(env: LostArkStoneEnv, solver: MCTS) -> Tuple[bool, int, State]:
    """한 에피소드 실행"""
    ...
    while not env.is_terminal(state):
        # MCTS로 다음 행동 결정
        best_state = solver.search(state)
        
        # 실제 환경에서 행동 실행
        action = None
        for a in ['A', 'B', 'C']:
            if env.get_next_state(state, a, True).a == best_state.a and \
               env.get_next_state(state, a, True).b == best_state.b and \
               env.get_next_state(state, a, True).c == best_state.c:
                action = a
                break
    ...
    return env.get_reward(state) > 0, steps, state
...
def main():
    ...
    for episode in range(n_episodes):
        success, steps, final_state = run_episode(env, solver)
