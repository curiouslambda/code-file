    ...
    if arg.make_cocoformat :
        ...
        if (arg.task == "all" or arg.task == "damage"):
            ...
            for l in l_sch:
                ...
                test.coco_json()
                tmp = list(label_df.loc[(label_df['dataset'] == 'test')&(label_df[l]>0)]['index'].values)
                test = RemakeCOCOformat(img_dir = dir_name_img, ann_dir = dir_name_label, data_lst = tmp, alis=f'damage_{l}_test', ratio=0., labeling_schme=l_sch, task='damage')
                test.coco_json()
                tmp = list(label_df.loc[(label_df['dataset'] == 'val')&(label_df[l]>0)]['index'].values)
                test = RemakeCOCOformat(img_dir = dir_name_img, ann_dir = dir_name_label, data_lst = tmp, alis=f'damage_{l}_val', ratio=0., labeling_schme=l_sch, task='damage')
                test.coco_json()
