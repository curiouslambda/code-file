...
def visualize_training(Q_table, steps_to_goal):
    # 하나의 figure 생성
    fig, axes = plt.subplots(1, 3, figsize=(15, 5))

    # 중간 진행 상황 시각화
    axes[0].plot(steps_to_goal)
    axes[0].set_xlabel('Episode')
    axes[0].set_ylabel('Steps to Goal')
    axes[0].set_title('Training Progress')

    # Q 테이블 시각화
    im = axes[1].imshow(np.max(Q_table, axis=2), cmap='viridis')
    axes[1].set_title('Q Table')
    ...
    axes[2].imshow(gridworld, cmap='terrain', interpolation='nearest')
    axes[2].set_title('Gridworld')

    # 결과 저장
    plt.savefig(f'plot/LR{LEARNING_RATE}__DF{DISCOUNT_FACTOR}__EPI{EPISODES}__EPS{EPSILON}.png')
    plt.show()
...
visualize_training(Q_table, steps_to_goal)